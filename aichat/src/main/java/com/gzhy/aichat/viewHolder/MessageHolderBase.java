...
import com.gzhy.aichat.adapter.MsgAdapter;
...
import com.gzhy.aichat.utils.SobotBitmapUtil;
import com.gzhy.aichat.utils.ToastUtil;
import com.gzhy.aichat.widget.ReSendDialog;
...
public abstract class MessageHolderBase {
    ...
    protected ChatMsgAdapter.MsgCallBack msgCallBack;
    ...
    public abstract void bindData(Context context, final ChatMessage message);
     ...
     * @param itemType
     */
    public void initNameAndFace(int itemType, Context context, final ChatMessage message,
                                String senderface, String sendername) {
        switch (itemType) {
            case ChatMsgAdapter.MSG_TYPE_IMG_R:
            case ChatMsgAdapter.MSG_TYPE_TXT_R:
                this.isRight = true;
                ...
                if (TextUtils.isEmpty(senderface)) {
                    ...
                    SobotBitmapUtil.displayRound(context, CommonUtil.encode(senderface), imgHead, defId);
                }

                name.setVisibility(TextUtils.isEmpty(sendername) ? View.GONE : View.VISIBLE);
                name.setText(sendername);
                break;
            case ChatMsgAdapter.MSG_TYPE_TXT_L:
            case ChatMsgAdapter.MSG_TYPE_IMG_L:
                this.isRight = false;
                //昵称、头像显示
                name.setVisibility(TextUtils.isEmpty(message.getSenderName()) ? View.GONE : View.VISIBLE);
                name.setText(message.getSenderName());
                SobotBitmapUtil.displayRound(context, CommonUtil.encode(message.getSenderFace()),
    ...
    public void setMsgCallBack(ChatMsgAdapter.MsgCallBack msgCallBack) {
